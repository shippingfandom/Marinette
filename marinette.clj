...
(defun mari-update-hacking-libraries () (begin
    ...
    (if (* has-aptclient (* is-network-active is-host)) (begin
            (gl-break-silence '[Mari] Updating repositories...')
            ((|> aptm) '-U')
                ...
                (if (== 'file' (typeof update-path)) (begin
                    (foreach _ library libraries (begin
                        ...
                        (if (!= 0 ((_ aptclient check_upgrade) aptclient lib-path)) (begin
                            (gl-break-silence (join (array '[Mari] Updating ' lib-path '...') ''))
                            ((|> aptm) '-i' library path))
                        (begin
                            (gl-break-silence (join (array '[Mari] Up to date: ' lib-path '!') ''))))))))))))))
...
(|= explorer (a1 a2 a3 a4)
    ...
    (begin
        ...
        (begin
            ...
            (gl-break-silence (join (array 'Opening FileExplorer.exe at ' path '...') ''))
    ...
    (begin
        ...
        (begin
            ...
            (if (== 0 path)
                ...
                (gl-break-silence (join (array 'Opening Notepad.exe for ' path '...') '')))
...
(|= view (a1 a2 a3 a4)
    ...
    (begin
        ...
        (begin
            ...
            (if (== 0 path)
                (gl-break-silence (join (array 'Opening ' graphical-program '...') ''))
                (gl-break-silence (join (array 'Opening ' graphical-program ' for ' path '...') '')))
...
(gl-break-silence '[Mari] Marinette v0.1.4 is successfully loaded! \\(^.^)/')
